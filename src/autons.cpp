...
void thirteen_awp() {
    ...
    chassis.moveToPoint(-45, -46.8, 2500, {.maxSpeed = 90, .minSpeed = 10, .earlyExitRange = 3});
    subsystems::matchload::extend();
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);
    ...
    chassis.moveToPoint(-71.25, 50, 1000, {.maxSpeed = 53, .minSpeed = 40, .earlyExitRange = 2});
    chassis.waitUntilDone();
    chassis.tank(-7, -7);
    pros::delay(100);
    chassis.tank(40,40);
...
void left_9_hold() {
    // 9 balls, preload + 3 mid + 2 under long, 3 matchloader, score in long goal
    // Tested:
    // location: normal left
...
void right() {
    // 7 balls, preload + 3 mid, 3 matchloader, score in long goal, wing long goal
    // Tested: works
    // Location: normal right
...
void rightElim(){
    ...
    chassis.moveToPoint(22, 21, 2000, {.maxSpeed = 110, .minSpeed = 90, .earlyExitRange = 1.5});
    chassis.waitUntil(13.5); // 15 to 13.5
    subsystems::matchload::extend();
    chassis.moveToPoint(32, 37.5, 1000, {.forwards = false, .maxSpeed = 90, .minSpeed = 70, .earlyExitRange = 1});
    chassis.waitUntilDone();
    chassis.moveToPoint(20, 37.5, 1000, {.forwards = false, .maxSpeed = 90, .minSpeed = 70, .earlyExitRange = 1});
    chassis.waitUntilDone();
    chassis.tank(-127, -127);
    subsystems::intake::run(subsystems::intake::GoalType::LONG_GOAL);
    pros::delay(1200);
...
void skillsFour() {
    ...
    */
    ...
    subsystems::intake::run(subsystems::intake::GoalType::MEDIUM_GOAL);
    pros::delay(1000); //1500 to 1000
    subsystems::intake::run(subsystems::intake::GoalType::MEDIUM_GOAL); // slow to normal
...
void skillsFive() {
    ...
    chassis.swingToHeading(180, lemlib::DriveSide::RIGHT, 1000, {.direction = lemlib::AngularDirection::CW_CLOCKWISE, .minSpeed = 30, .earlyExitRange = 3});
    // clear park zone
    chassis.moveToPoint(70, 4.13, 2000, {.maxSpeed = 110, .minSpeed = 105, .earlyExitRange = 1.5}); //try 4.25 to 5 if doesn't work
    chassis.waitUntilDone();
    pros::delay(100000);
}
...
void hoodPush_100() {
    chassis.setPose(28, 44, 90);
    // hood push
    chassis.moveToPoint(30.5, 44, 1100, {.maxSpeed = 75, .minSpeed = 60, .earlyExitRange = 0.5});
    chassis.moveToPoint(20, 44, 1200, {.forwards = false, .maxSpeed = 15});
    chassis.waitUntilDone();
}
...
void skills_2(){
    ...
    chassis.waitUntil(20);
    chassis.cancelMotion();
    chassis.moveToPoint(65.5, 47, 1100, {.maxSpeed = 45, .minSpeed = 40, .earlyExitRange = 1.5});

    chassis.waitUntilDone();

    // into matchloader 2
    chassis.tank(50,50); // both nums to 25 from 20
    pros::delay(900); //700 to 600
    chassis.tank(0,0);
...
void skills_3() {
    ...
    /*
        ...
        chassis.moveToPose(35.85, -21.15, 270, 4000, {.maxSpeed = 100});
        chassis.waitUntil(10);
        chassis.cancelMotion();

        chassis.moveToPose(32.5, -17, 225, 4000, {.maxSpeed = 95});
        chassis.waitUntil(3);
        chassis.cancelMotion();
        chassis.moveToPose(32.5, -17, 225, 4000, {.maxSpeed = 45}); //22.25 to -20
        chassis.waitUntilDone();

        chassis.tank(-100,-100);
        pros::delay(125);

        // chassis.moveToPoint(45, -20, 4000, {.forwards = false});

        chassis.moveToPoint(20, 0, 2000); //-5 to 0
        ...
        chassis.moveToPoint(-2, -23.75, 2000, {.forwards = false, .maxSpeed = 40, .minSpeed = 10, .earlyExitRange = 1});
        pros::delay(500);
        chassis.cancelMotion();
        ...
        chassis.moveToPose(10.1, -9.75, 135, 2000, {.forwards = false, .lead = 0.85, .maxSpeed = 55, .minSpeed = 20, .earlyExitRange = 1}); //.88 to .8 and 11 to 10
        chassis.waitUntilDone();
        chassis.tank(-30, -30);
        pros::delay(200);
        subsystems::matchload::extend();
        ...
        pros::delay(800); //1000 to 800

        // subsystems::intake::run(subsystems::intake::GoalType::LOW_GOAL);
        // pros::delay(100);
        subsystems::intake::run(subsystems::intake::GoalType::MEDIUM_GOAL);
        pros::delay(850); //1000 to 900
        subsystems::intake::run(subsystems::intake::GoalType::MEDIUM_GOAL_SLOW);
        pros::delay(450); //1000 to 900
        chassis.tank(2, 2);
        pros::delay(400);
        // 3600
        ...
        chassis.moveToPoint(50, -45.3, 1500, {.minSpeed = 60, .earlyExitRange = 1.5}); //2000 to 1200
        ...
        chassis.moveToPoint(20, -45.7, 1500, {.forwards = false, .minSpeed = 60, .earlyExitRange = 1.5}); // no max speed
        ...
        chassis.tank(-127, -127);
        pros::delay(400);
        chassis.setPose(28, -48, chassis.getPose().theta); //44 to 48
        subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);

        chassis.moveToPoint(61, -49,  1000, {.maxSpeed = 65, .minSpeed = 60, .earlyExitRange = 1.5}); //-48 to -46.5
    ...
    */
    ...
    chassis.turnToPoint(68, -51, 1600, {.maxSpeed = 85, .minSpeed = 60, .earlyExitRange = 1.5}); //44.5 to 46

    chassis.moveToPoint(20, -51, 1000, {.forwards = false, .maxSpeed = 70, .minSpeed = 60, .earlyExitRange = 1.5}); //44.5 to 46
    chassis.waitUntil(13);
    chassis.tank(-127, -127);
    subsystems::intake::run(subsystems::intake::GoalType::LONG_GOAL);
    chassis.waitUntilDone();
    ...
    chassis.moveToPose(64, -51, 90,  2000, {.maxSpeed = 65, .minSpeed = 60, .earlyExitRange = 1.5});
    ...
    chassis.tank(55, 55);
    // into 3rd matchloader
    pros::delay(1500); //2000 to 1700
    chassis.tank(0,0);

    chassis.moveToPoint(50, -50, 2000, {.forwards = false, .maxSpeed = 70, .minSpeed = 50, .earlyExitRange = 1});
    chassis.waitUntilDone();
    chassis.moveToPose(29, -64, 90, 2000, {.forwards = false, .lead = 0.3, .minSpeed = 50, .earlyExitRange = 1}); //.maxSpeed = 110,
    chassis.waitUntilDone();
    subsystems::matchload::retract();
    chassis.moveToPoint(-29, -64, 3000, {.forwards = false, .minSpeed = 65, .earlyExitRange = 3}); // -30 to -31 and .maxSpeed = 120, 
    chassis.waitUntilDone();
    chassis.moveToPoint(-29, -50.5, 2000, {.forwards = false, .minSpeed = 50, .earlyExitRange = 0.25}); // 52 to 50.5 and -30 to -31 and .maxSpeed = 70,
    chassis.waitUntilDone();
    chassis.moveToPoint(-15, -50.5, 1000, {.forwards = false}); // 52 to 50.5
    chassis.waitUntilDone();
...
void skills_4() {
    ...
    // move to clear park zone
    chassis.moveToPoint(61.5, 30, 1800, {.maxSpeed = 65, .minSpeed = 30, .earlyExitRange = 1.5}); //2000 to 1800
    chassis.waitUntilDone();

    chassis.swingToHeading(180, lemlib::DriveSide::RIGHT, 900, {.direction = lemlib::AngularDirection::CW_CLOCKWISE, .minSpeed = 30, .earlyExitRange = 3}); //1000 to 900
    // clear park zone
    chassis.moveToPoint(70, 4.13, 2000, {.maxSpeed = 110, .minSpeed = 105, .earlyExitRange = 1.5}); //try 4.25 to 5 if doesn't work
    chassis.waitUntilDone();
    pros::delay(100000);
...
void skills_1() {
    ...
    //start
    chassis.setPose(-48, 14.5, 90);
    // chassis.moveToPose(-30.25, 16, 90, 1100, {.maxSpeed = 45}); //1800 to 1000 and 17 to 16, -30.35 to -30.35
    chassis.moveToPose(-28.5, 13, 45, 2000, {.lead = .4, .maxSpeed = 65});
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);
    chassis.waitUntilDone();
    ...
    // chassis.moveToPoint(-40, 16, 1000, {.forwards = false});
    chassis.moveToPoint(-20, 0, 2000,{.forwards = false}); // -20 to -18
    // pros::delay(1500);

    chassis.moveToPoint(2, 23.75, 2000, {.forwards = false, .maxSpeed = 60, .minSpeed = 10, .earlyExitRange = 1});
    pros::delay(700);
    chassis.cancelMotion();

    // pros::delay(1500);

    chassis.moveToPose(-7, 9.5, 315, 2000, {.forwards = false, .lead = 0.88, .maxSpeed = 80}); //.6 to .55
    ...
    //move away from center goal
    chassis.moveToPoint(-40, 45.2, 2500); //80 to 110 and 43.5 to 43
    chassis.waitUntil(10);

    subsystems::matchload::extend();
    chassis.waitUntilDone();

    chassis.turnToPoint(-68, 46, 1600, {.maxSpeed = 60, .minSpeed = 60, .earlyExitRange = 1.5}); //46 to 45
    chassis.waitUntilDone();

    chassis.moveToPoint(-20, 45, 1000, {.forwards = false, .minSpeed = 60, .earlyExitRange = 1.5}); // no maxspeed
    ...
    chassis.moveToPoint(-67.75, 45, 1000, {.maxSpeed = 60, .minSpeed = 60, .earlyExitRange = 1.5}); //-68 to -67.75
    subsystems::wing::toggle();

    chassis.waitUntil(10);
    chassis.cancelMotion();
    chassis.moveToPoint(-67, 45, 900, {.maxSpeed = 50, .minSpeed = 40, .earlyExitRange = 1.5}); //-67.5 to -67
    ...
    pros::delay(200);
    chassis.tank(0,0);
    subsystems::wing::extend();//added
    // out of first matchloader
    chassis.moveToPoint(-58, 44.5, 1500, {.forwards = false, .maxSpeed = 80, .earlyExitRange = 0.5});
    chassis.moveToPoint(-35, 55.5, 1500, {.forwards = false, .maxSpeed = 127, .minSpeed = 40, .earlyExitRange = 1}); //90 to 127

    // chassis.moveToPoint(-40, 55.5, 500, {.minSpeed = 127, .earlyExitRange = 1});//added
    // chassis.waitUntilDone();//added

    chassis.moveToPoint(29, 57, 2000, {.forwards = false, .maxSpeed = 127, .minSpeed = 40, .earlyExitRange = 1.5}); //100 to 127
    subsystems::matchload::retract();

    // turn to get into long goal scoring

    chassis.moveToPoint(36, 46, 1500, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 1.5}); //44 to 45

    chassis.moveToPoint(20, 46, 800, {.forwards = false, .maxSpeed = 90, .minSpeed = 50,  .earlyExitRange = 0.5}); //1050 to 850 //44 to 45
    
    subsystems::matchload::extend();

    chassis.waitUntilDone();
    subsystems::wing::retract();//added
    // into long goal 1
    chassis.tank(50,50);
    // pros::delay(75);
    subsystems::intake::run(subsystems::intake::GoalType::LONG_GOAL);
...
void skills_2() {
    ...
    chassis.waitUntil(20);
    chassis.cancelMotion();
    chassis.moveToPoint(66.75, 47, 1100, {.maxSpeed = 50, .minSpeed = 40, .earlyExitRange = 1.5}); //67 to 66.75

    chassis.waitUntilDone();

    // into matchloader 2
    chassis.tank(35, 35); //25 to 30
...
void skills_3() {
    ...
    // move to clear park zone
    chassis.moveToPoint(59.5, 30, 2000, {.maxSpeed = 100}); //65 to 100
    chassis.waitUntilDone();
    ...
    chassis.moveToPose(50, -45.3, 90, 1500, {.lead = 0.2}); //2000 to 1200
    ...
    chassis.moveToPose(20, -45.7, 90, 1500, {.forwards = false, .minSpeed = 60, .earlyExitRange = 1.5}); // no max speed
    ...
    chassis.tank(-127, -127);
    pros::delay(300);
    chassis.setPose(28, -48, 90); //44 to 48
    pros::delay(100);
    subsystems::intake::run(subsystems::intake::GoalType::HOLD_BALLS);

    chassis.moveToPose(61, -46.75, 90,  1000, {.maxSpeed = 65, .minSpeed = 60, .earlyExitRange = 1.5}); //-48 to -46.5

    chassis.waitUntil(10);
    chassis.cancelMotion();
    chassis.moveToPose(62.5, -46.75, 90, 1000, {.maxSpeed = 60, .minSpeed = 50, .earlyExitRange = 1.5}); //-48 to -46.5

    chassis.waitUntilDone();
    chassis.tank(35, 35);
    // into 3rd matchloader
    pros::delay(1000); //1700 to 700
    chassis.tank(0,0);

    chassis.moveToPoint(50, -50, 1000, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 1}); //2000 to 1000 and 70 to 90
    chassis.waitUntilDone();
    chassis.moveToPose(29, -58, 90, 1000, {.forwards = false, .lead = 0.3, .maxSpeed = 127, .minSpeed = 50, .earlyExitRange = 1}); //2000 to 1000 and 110 to 127 and -60 to -59
    chassis.waitUntilDone();
    subsystems::matchload::retract();
    chassis.moveToPoint(-31, -58, 2500, {.forwards = false, .maxSpeed = 127, .minSpeed = 50, .earlyExitRange = 3}); //3000 to 2500 and 110 to 127 and -60 to -59
    chassis.waitUntilDone();
    chassis.moveToPoint(-31, -44.5, 1500, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 0.25}); //2000 to 1500 and 70 to 90 and -50.5 to -49.5
    chassis.waitUntilDone();
    chassis.moveToPoint(-15, -44.5, 600, {.forwards = false}); //1000 to 600 and -50.5 to -49.5
    chassis.waitUntilDone();
...
void skills_4() {
    ...
    chassis.swingToHeading(180, lemlib::DriveSide::RIGHT, 1000, {.direction = lemlib::AngularDirection::CW_CLOCKWISE, .minSpeed = 30, .earlyExitRange = 3});
    // clear park zone
    chassis.moveToPoint(70, 8, 2500, {.maxSpeed = 110, .minSpeed = 105, .earlyExitRange = 1.5}); // 6 to 8 bc didn't go far enough
    chassis.waitUntilDone();
    pros::delay(100000);
}
*/